...
import { applyDiscountToPresetPrice, resolvePresetEffectivePrice } from '@/lib/preset-pricing'
...
type DurationMode = 'duration' | 'end_date' | 'preset'
...
function getDefaultEndDate(timeZone: string, days = 1) {
  const next = new Date()
  next.setSeconds(0, 0) // truncate sub-minute precision to reduce duration drift
...
function formatDurationDays(days: number): string {
  ...
  return parts.length > 0 ? parts.join(' ') : `${days.toFixed(3)}`
}
...
export function CustomerCreatePage({ title, description, backPath, createCustomer }: CustomerCreatePageProps) {
  ...
  const [mode, setMode] = useState<DurationMode>('end_date')
  ...
  const activeOfferDiscount: number | null = (selectedProgram as any)?.active_offer_discount ?? null
  const selectedPresetPricingWithDiscount = useMemo(
    () => applyDiscountToPresetPrice(selectedPresetPricing, activeOfferDiscount),
    [selectedPresetPricing, activeOfferDiscount],
  )
  ...
  }, [availablePresets, isPresetSeller, mode])
  ...
  const durationDays = useMemo(() => {
    ...
    if (mode === 'end_date') {
      if (!endDate) return 0
      const zonedEndDate = zonedDateTimeInputToUtcDate(endDate, endTimezone)
  ...
  }, [createLicenseNow, durationUnit, durationValue, effectiveStartDate, endDate, endTimezone, isPresetSeller, mode, selectedPreset?.duration_days])

  const autoPrice = useMemo(() => {
    if (!selectedProgram || !createLicenseNow) return 0
    if (isPresetSeller || mode === 'preset') return selectedPresetPricingWithDiscount.effectivePrice
  ...
  }, [createLicenseNow, durationDays, isPresetSeller, mode, selectedPresetPricingWithDiscount.effectivePrice, selectedProgram])
  ...
  const totalPrice = useMemo(() => {
    if (isPresetSeller || mode === 'preset' || priceMode === 'auto') return autoPrice
    const parsed = Number(priceInput)
    return Number.isFinite(parsed) && parsed >= 0 ? parsed : 0
  }, [autoPrice, isPresetSeller, mode, priceInput, priceMode])
  ...
  const expiryPreview = useMemo(() => {
    ...
    return new Date(effectiveStartDate.getTime() + durationDays * 86400000).toISOString()
  }, [createLicenseNow, durationDays, effectiveStartDate, endDate, endTimezone, isPresetSeller, mode])
  ...
  const endSummary = useMemo(() => {
    ...
    const displayTz = isPresetSeller || mode !== 'end_date' ? scheduleTimezone : endTimezone
    return `${t('activate.endingDate', { defaultValue: 'Ending date' })}: ${formatDate(expiryPreview, locale, displayTz)}`
  }, [createLicenseNow, endTimezone, expiryPreview, isPresetSeller, locale, mode, scheduleTimezone, t])
  ...
  const activateMutation = useMutation({
    mutationFn: () => activateLicense({
      ...
      duration_days: (isPresetSeller || mode === 'preset') ? undefined : Number(durationDays.toFixed(6)),
      price: (isPresetSeller || mode === 'preset') ? undefined : totalPrice,
            ...
            <>
              ...
              <div className="space-y-3 rounded-2xl border border-slate-200 p-4 dark:border-slate-700">
                ...
                {isPresetSeller ? (
                  <div className="space-y-2">
                    <Label>{t('software.durationPresetsTitle', { defaultValue: 'Duration Presets' })}</Label>
                    <div className="flex flex-wrap gap-2">
                    ...
                    {selectedPreset ? (
                      <p className="text-sm text-slate-500 dark:text-slate-400">
                        {(function(d: number) {
                              ...
                              return `${d} days`
                            })(selectedPreset.duration_days)} • {t('activate.presetPriceSummary', { price: selectedPresetPricingWithDiscount.effectivePrice.toFixed(2), defaultValue: '$ {{price}}' })}
                      </p>
                    ) : null}
                  </div>
                ) : (
                  <>
                    ...
                    {mode === 'preset' ? (
                        ...
                        {selectedPreset ? (
                          <p className="text-sm text-slate-500 dark:text-slate-400">
                            {(function(d: number) {
                              ...
                              if (d % 7 === 0) return `${d / 7} week${d / 7 > 1 ? 's' : ''}`
                              return `${d} days`
                            })(selectedPreset.duration_days)} • {formatCurrency(selectedPresetPricingWithDiscount.effectivePrice, 'USD', locale)}
                          </p>
                        ) : null}
                      </div>
                    ) : mode === 'duration' ? (
                      ...
                      <div className="space-y-2">
                        ...
                        <select value={endTimezone} onChange={(event) => setEndTimezone(event.target.value)} className="h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm dark:border-slate-700 dark:bg-slate-950">
                          {COMMON_TIMEZONES.map((timezone) => (
                            <option key={timezone.value} value={timezone.value}>{timezone.label}</option>
                          ))}
                        </select>
              ...
              <div className="space-y-3 rounded-2xl border border-slate-200 p-4 dark:border-slate-700">
                <div className="flex items-center justify-between gap-3">
                  ...
                  {isPresetSeller || mode === 'preset' ? null : (
                ...
                <Input
                  value={priceInput}
                  disabled={isPresetSeller || mode === 'preset'}
                  readOnly={isPresetSeller || mode === 'preset' || priceMode === 'auto'}
                  onChange={(event) => setPriceInput(event.target.value.replace(/[^\d.]/g, ''))}
                  className={isPresetSeller || mode === 'preset' || priceMode === 'auto' ? 'cursor-not-allowed bg-slate-100 text-slate-500 dark:bg-slate-900 dark:text-slate-400' : ''}
                />
                <p className="text-sm text-slate-500 dark:text-slate-400">
                  {isPresetSeller || mode === 'preset'
                ...
                <div className="grid gap-3 md:grid-cols-3">
                  <Summary label={t('activate.durationDays', { defaultValue: 'Duration in Days' })} value={formatDurationDays(durationDays)} />
                  <Summary label={t('activate.expiryPreview', { defaultValue: 'Expiry Preview' })} value={expiryPreview ? formatDate(expiryPreview, locale, isPresetSeller || mode !== 'end_date' ? scheduleTimezone : endTimezone) : '-'} />
