  ...
  return (
    <>
      ...
      <main
        ...
        tabIndex={-1}
        className="flex min-h-screen items-center justify-center bg-[radial-gradient(circle_at_top,_rgba(14,165,233,0.18),_transparent_35%),linear-gradient(180deg,_#f8fafc,_#e2e8f0)] px-4 py-10 focus:outline-none dark:bg-[radial-gradient(circle_at_top,_rgba(14,165,233,0.18),_transparent_35%),linear-gradient(180deg,_#020617,_#0f172a)]"
      >
        <PageTransition transitionKey={`login-${lang}`} className={`w-full max-w-5xl ${isRtl ? 'text-right' : 'text-left'}`}>
          <div className="mb-8 flex items-center justify-between gap-4">
            ...
            <Button variant="outline" type="button" onClick={switchLanguage}>
              {lang === 'ar' ? 'EN' : 'AR'}
            </Button>
          </div>
            ...
            <section className="space-y-6 rounded-3xl border border-white/40 bg-white/60 p-8 backdrop-blur dark:border-slate-800 dark:bg-slate-900/50">
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                {['secure', 'roles', 'speed'].map((key) => (
                  <div key={key} className="rounded-2xl border border-slate-200 bg-white/90 p-4 transition-shadow duration-200 hover:shadow-lg dark:border-slate-800 dark:bg-slate-950/70">
            ...
            <Card>
              ...
              <CardContent>
                <form className="space-y-5" onSubmit={handleSubmit}>
                  ...
                  <button
                    className="text-sm font-medium text-sky-600 transition hover:text-sky-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 dark:text-sky-400"
