...
export function LicensesPage() {
  ...
  const columns = useMemo<Array<DataTableColumn<LicenseSummary>>>(
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              {canRetryScheduledLicense(row) ? (
                <DropdownMenuItem onClick={() => retryScheduledMutation.mutate(row.id)} disabled={retryScheduledMutation.isPending}>
    ...
    [allVisibleSelected, lang, locale, selectedIds, someVisibleSelected, t, visibleIds, resumeMutation.isPending, retryScheduledMutation.isPending],
  )
  ...
  return (
    <div className="space-y-6">
        ...
        <TabsContent value={status} className="space-y-4">
          <Card>
            <CardContent className="grid gap-3 p-4 lg:grid-cols-[minmax(0,1fr)_220px]">
              ...
              <select value={resellerId} onChange={(event) => { setResellerId(event.target.value ? Number(event.target.value) : ''); setPage(1); setSelectedIds([]) }} className="h-11 rounded-xl border border-slate-300 bg-white px-3 text-sm dark:border-slate-700 dark:bg-slate-950">
      ...
      <RenewLicenseDialog
        ...
        initialExpiresAt={renewTarget?.expires_at}
