...
import type { ManagerParentBiosChangeRequest } from '@/types/manager-parent.types'
...
export function BiosChangeRequestsPage() {
  ...
  const [rejectTarget, setRejectTarget] = useState<ManagerParentBiosChangeRequest | null>(null)
  ...
  const approveMutation = useMutation({
    mutationFn: (id: number) => managerParentService.approveBiosChangeRequest(id),
    onSuccess: (response) => {
      toast.success(response.message ?? t('biosChangeRequests.approved'))
      void queryClient.invalidateQueries({ queryKey: ['manager-parent', 'bios-change-requests'] })
    },
    onError: (error) => toast.error(resolveApiErrorMessage(error, t('common.error'))),
  })

  const rejectMutation = useMutation({
    mutationFn: ({ id, notes }: { id: number; notes: string }) => managerParentService.rejectBiosChangeRequest(id, notes),
    onSuccess: (response) => {
  ...
  const columns = useMemo<Array<DataTableColumn<ManagerParentBiosChangeRequest>>>(() => [
    {
      key: 'customer_name',
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        tableKey="manager_parent_bios_change_requests"
        columns={columns}
...
function StatusPill({ status, t }: { status: ManagerParentBiosChangeRequest['status']; t: (key: string) => string }) {
  const styles: Record<ManagerParentBiosChangeRequest['status'], string> = {
  ...
  const labels: Record<ManagerParentBiosChangeRequest['status'], string> = {
