...
export function ResellerLogsPage() {
  ...
  const logsQuery = useQuery({
    ...
    queryFn: () => managerService.getSellerLogs({ page, per_page: perPage, seller_id: sellerId, action, from: range.from, to: range.to }),
  })

  const teamQuery = useQuery({
    queryKey: ['manager', 'seller-logs', 'team'],
    queryFn: () => managerService.getTeam({ per_page: 100 }),
  })

  const sellerOptions = useMemo<SellerOption[]>(() => {
    const options: SellerOption[] = (teamQuery.data?.data ?? [])
      .map((member: ManagerTeamReseller) => ({ id: member.id, name: member.name, role: 'reseller' as const }))

    if (user && user.role === 'manager' && !options.some((member) => member.id === user.id)) {
  ...
  const columns = useMemo<Array<DataTableColumn<ManagerSellerLogEntry>>>(() => [
    {
      key: 'created_at',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
        <StatusFilterCard label={t('manager.pages.activity.allActions')} count={summary.total_entries} isActive={action === ''} onClick={() => { setAction(''); setPage(1) }} color="sky" />
