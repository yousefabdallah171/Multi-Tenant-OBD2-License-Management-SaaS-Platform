...
export function BiosDetailsPage() {
  ...
  const searchQuery = useQuery({
    queryKey: ['manager', 'bios-details', 'search', debouncedSearch],
    queryFn: () => managerBiosDetailsService.searchBiosIds(debouncedSearch),
  ...
  const normalizedActivity = useMemo(() => dedupeBiosActivity(activityQuery.data ?? []), [activityQuery.data])
            ...
            <div className="flex flex-wrap gap-2">
              {visibleBiosList.map((candidate) => (
                <button
                  ...
                  onClick={() => navigate(routePaths.manager.biosDetail(lang, candidate))}
                >
                  {candidate}
                </button>
              ))}
      ...
      {biosId ? (
        <Tabs defaultValue="overview" className="space-y-4">
          <TabsList className="flex flex-wrap">
            ...
            <TabsTrigger value="activity">{t('manager.nav.activity')}</TabsTrigger>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <SectionCard title={t('biosDetails.customer')}>
                  <InfoGrid items={[
                    [t('common.customer'), latestCustomer?.id ? <Link className="text-sky-600 hover:underline dark:text-sky-300" to={routePaths.manager.customerDetail(lang, latestCustomer.id)}>{latestCustomer.name}</Link> : (latestCustomer?.name ?? '-')],
                    [t('common.email'), latestCustomer?.email ?? '-'],
                    [t('common.reseller'), latestReseller?.id ? <Link className="text-sky-600 hover:underline dark:text-sky-300" to={routePaths.manager.teamMemberDetail(lang, latestReseller.id)}>{latestReseller.name}</Link> : (latestReseller?.name ?? '-')],
...
function dedupeBiosActivity(items: BiosActivity[]) {
  const seen = new Set<string>()
  return items.flatMap((item) => {
    const family = resolveActivityFamily(item.action)
...
function resolveActivityFamily(action: string) {
