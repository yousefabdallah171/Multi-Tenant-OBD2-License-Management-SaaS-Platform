...
export function IpAnalyticsPage() {
  ...
  const logsQuery = useQuery({
    queryKey: ['super-admin', 'ip-analytics', page, perPage, search, reputation, range.from, range.to, country],
    queryFn: () => superAdminPlatformService.getIpAnalytics({
      page,
      per_page: perPage,
  ...
  const columns = useMemo<Array<DataTableColumn<SoftwareIpRow>>>(() => [
    {
      ...
      render: (row) => {
        if (row.customer_id) {
          return (
            <Link className="text-sky-600 hover:underline" to={routePaths.superAdmin.customerDetail(lang, row.customer_id)}>
        ...
        if (row.reseller_id) {
          return (
            <Link className="text-sky-600 hover:underline" to={routePaths.superAdmin.userDetail(lang, row.reseller_id)}>
    ...
    },
    {
      ...
      render: (row) => row.bios_id ? (
        <Link className="text-sky-600 hover:underline" to={routePaths.superAdmin.biosDetail(lang, row.bios_id)}>
  ...
  return (
    <div className="space-y-6">
      ...
      {rows.length === 0 && !logsQuery.isLoading && !logsQuery.isFetching ? (
        ...
        <DataTable
          tableKey="super_admin_ip_analytics"
          columns={columns}
          data={rows}
          rowKey={(row) => row.id}
