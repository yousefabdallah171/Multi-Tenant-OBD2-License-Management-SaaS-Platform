...
export function ResellerLogsPage() {
  ...
  const logsQuery = useQuery({
    queryKey: ['super-admin', 'seller-logs', page, perPage, sellerId, action, range.from, range.to],
    queryFn: () => superAdminPlatformService.getSellerLogs({ page, per_page: perPage, seller_id: sellerId, action, from: range.from, to: range.to }),
  })

  const teamQuery = useQuery({
    queryKey: ['super-admin', 'seller-logs', 'team'],
    queryFn: () => superAdminPlatformService.getSellers({ per_page: 100 }),
  })
          ...
          <div className="space-y-1">
            {row.seller.id && (role === 'manager' || role === 'reseller') ? (
              <Link
                ...
                to={routePaths.superAdmin.userDetail(lang, row.seller.id)}
    ...
    },
    {
      ...
      render: (row) => {
        ...
        return (
          <Link
            className="text-sky-600 hover:underline dark:text-sky-300"
            to={routePaths.superAdmin.biosDetail(lang, biosId)}
            onClick={(event) => event.stopPropagation()}
          >
            {biosId}
          </Link>
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        tableKey="super_admin_reseller_logs"
        columns={columns}
        ...
        onRowClick={(row) => {
          ...
          if (row.seller?.id && (role === 'manager' || role === 'reseller')) {
            navigate(routePaths.superAdmin.userDetail(lang, row.seller.id))
          }
        }}
        pagination={{
