...
export function ProgramLogsPage() {
  ...
  const programsQuery = useQuery({
    queryKey: ['super-admin', 'programs-with-external-api'],
  ...
  const logsQuery = useQuery({
    queryKey: ['super-admin', 'program-logs', selectedProgramId, page, perPage, sellerId, actions, autoRefresh],
    queryFn: () => {
      ...
      return superAdminPlatformService.getProgramLogs(selectedProgramId, {
        page,
        per_page: perPage,
  ...
  const userColumns = useMemo<Array<DataTableColumn<ProgramUserLogEntry>>>(() => [
    ...
    },
    {
      ...
      render: (row) => row.bios_id ? (
        <Link className="text-sky-600 hover:underline dark:text-sky-300" to={routePaths.superAdmin.biosDetail(lang, row.bios_id)}>
          {row.bios_id}
        </Link>
      ) : '-',
    },
  ...
  const loginColumns = useMemo<Array<DataTableColumn<ProgramLog>>>(() => [
    {
      ...
      render: (row) => row.customer_id ? (
        <Link className="text-blue-600 hover:underline dark:text-blue-300" to={routePaths.superAdmin.customerDetail(lang, row.customer_id)}>
          @{row.username}
        </Link>
      ) : (
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'users' | 'login')}>
        ...
        <TabsContent value="users">
          <DataTable
            tableKey="super_admin_program_logs_user_actions"
            columns={userColumns}
            data={userRows}
            rowKey={(row) => row.id}
          ...
          <Card>
            <CardContent className="p-4">
              {loginRows.length === 0 ? (
                ...
                <DataTable
                  tableKey="super_admin_program_logs_login_events"
