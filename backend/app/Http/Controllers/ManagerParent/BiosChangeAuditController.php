...
class BiosChangeAuditController extends BaseManagerParentController
{
    public function summary(Request $request): JsonResponse
    {
        $tenantId = $this->currentTenantId($request);
        $managerIds = $this->tenantManagerIds($tenantId);
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        if (
            ($validated['type'] ?? null) !== 'direct_change'
            && ! in_array($statusFilter, $directStatuses, true)
        ) {
            $items = $items->merge($this->requestRows($tenantId, $validated));
        }

        if (
            ($validated['type'] ?? null) !== 'request'
            && ! in_array($statusFilter, $requestStatuses, true)
        ) {
    ...
    private function requestRows(int $tenantId, array $validated): Collection
    {
        ...
        return BiosChangeRequest::query()
            ...
            ->with([
                'license:id,customer_id,program_id',
            ...
            ->get()
            ->map(fn (BiosChangeRequest $entry): array => $this->serializeRequestRow($entry));
    }
    ...
    private function directChangeRows(int $tenantId, Collection $managerIds, array $validated): Collection
    {
        ...
        $licenseIds = $logs
            ->map(fn (ActivityLog $log): int => (int) ($log->metadata['license_id'] ?? 0))
            ->filter()
            ->unique()
            ->values();

        $customerIds = $logs
            ->map(fn (ActivityLog $log): int => (int) ($log->metadata['customer_id'] ?? 0))
            ->filter()
            ->unique()
            ->values();

        $programIds = $logs
            ->map(fn (ActivityLog $log): int => (int) ($log->metadata['program_id'] ?? 0))
            ->filter()
            ->unique()
            ->values();

        $resellerIds = $logs
            ->map(fn (ActivityLog $log): int => (int) ($log->metadata['reseller_id'] ?? 0))
            ->filter()
            ->unique()
            ->values();

        $licenses = License::query()
            ->select(['id', 'customer_id', 'program_id', 'reseller_id'])
            ->with(['customer:id,name', 'program:id,name', 'reseller:id,name'])
            ->where('tenant_id', $tenantId)
        ...
        return $logs
            ->map(function (ActivityLog $log) use ($licenses, $customers, $programs, $resellers): array {
                ...
                $reseller = $resellers->get((int) ($metadata['reseller_id'] ?? 0));

                return $this->serializeDirectRow($log, $license, $customer, $program, $reseller);
            })
            ->when(
    ...
    private function serializeRequestRow(BiosChangeRequest $entry): array
    {
        return [
            'id' => 'request-'.$entry->id,
            'type' => 'request',
    ...
    private function serializeDirectRow(
        ActivityLog $log,
        ?License $license,
        ?User $customer,
        ?Program $program,
        ...
        return [
            ...
            'status' => $this->directStatus($log->action),
            'reason' => null,
            'reviewer_notes' => null,
    ...
    private function tenantManagerIds(int $tenantId): Collection
    {
        return User::query()
            ->where('tenant_id', $tenantId)
    ...
    private function directStatus(string $action): string
    {
        return $action === 'bios.direct_change_failed' ? 'failed' : 'completed';
    }
