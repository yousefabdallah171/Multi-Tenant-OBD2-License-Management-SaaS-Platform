...
class IpAnalyticsController extends BaseSuperAdminController
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $cacheKey = 'super-admin:ip-analytics:matched:'.($tenantId ?? 'all');
        ...
        if (! is_array($matched)) {
            ...
            $tenantIds = $tenantId ? [$tenantId] : Tenant::query()->pluck('id')->map(fn ($id): int => (int) $id)->all();
            $matched = [];

            foreach ($tenantIds as $currentTenantId) {
                foreach ($this->ipAnalyticsService->matchLogsToDatabaseRecords($parsed, $currentTenantId) as $row) {
                    if (($row['program_id'] ?? null) === null) {
                        continue;
                    }
                    ...
                    $matched[] = [
                        ...
                        'tenant_name' => Tenant::query()->whereKey($currentTenantId)->value('name'),
                    ];
                }
            }
        ...
        if (! empty($validated['search'])) {
            ...
            $rows = $rows->filter(fn (array $row): bool => str_contains(mb_strtolower((string) ($row['username'] ?? '')), $search)
                ...
                || str_contains(mb_strtolower((string) ($row['tenant_name'] ?? '')), $search)
